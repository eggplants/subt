...
from translatepy import language, translators  # type: ignore[import-untyped]
from translatepy.utils.importer import get_translator  # type: ignore[import-untyped]
...
def __get_services() -> tuple[str, ...]:
    return tuple(
        service
        for service in translators.__dir__()
        if "__" not in service and service != "base"
    )
